...
export default function MedicinesAdmin({ onMessage }: MedicinesAdminProps): React.JSX.Element {
  ...
  const handleCreateItem = async (data: any) => {
    ...
    dispatch(fetchMedicines({ page: currentPage, limit: pageSize }))
    onMessage('Medicine created successfully')
  }

  const handleUpdateItem = async (data: any) => {
    await dispatch(updateMedicine(data))
    setEditingItem(null)
    onMessage('Medicine updated successfully')
  }
  ...
  const handleDeleteItem = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this medicine?')) {
      ...
      onMessage('Medicine deleted successfully')
    }
  }
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold">Medicines ({medicines.length})</h3>
        <button
      ...
      <Modal
        ...
        onClose={closeModal}
        title={editingItem ? 'Edit Medicine' : 'Create Medicine'}
        maxWidth="md"
      >
        <form onSubmit={handleFormSubmit}>
          <div className="space-y-4">
            <FormField
              ...
              required
              placeholder="Lijek..."
              helpText="Enter the medicine name in Montenegrin"
            />
            ...
            <FormField
              ...
              helpText="Enter the medicine name in English (optional)"
            />
            ...
            <FormField
              ...
              rows={4}
              helpText="Provide information about usage, dosage, or other relevant details"
            />
          ...
          <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-gray-200">
            ...
            >
              {editingItem ? 'Update Medicine' : 'Create Medicine'}
            </button>
          </div>
        </form>
      </Modal>
