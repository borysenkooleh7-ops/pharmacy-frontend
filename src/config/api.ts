...
class ApiService {
  ...
  async getCities(): Promise<City[]> {
    const response = await this.request<ApiResponse<City[]>>('/cities', { includeAuth: false })
  ...
  async getPharmacies(params: Record<string, any> = {}): Promise<Pharmacy[]> {
    ...
    const response = await this.request<ApiResponse<Pharmacy[]>>(`/pharmacies?${queryParams}`, { includeAuth: false })
    return response.data || []
  }
